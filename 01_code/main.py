...
def main():
    ...
    api_token = "YOUR_SONG_GENERATION_API_TOKEN"
    ...
    context = (
        ...
        "my back. The betrayal stung more than I thought it could, so I decided to play some Shawn Mendes. I felt "
    ...
    generated_prompt, singer_name, music_genre = generate_music_details(context)
    ...
    if generated_prompt and singer_name and music_genre:
        ...
        workId = generate_song_request(api_token, generated_prompt, gpt_description_prompt)
        
        if workId:
            # Step 2: Poll the status until the song is ready
            audio_url = poll_song_status(api_token, workId)
            if audio_url:
